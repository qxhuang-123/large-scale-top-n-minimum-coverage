"""
Gurobi exact solver for the OP Top-N recommendation problem.

Default setting:
  - dataset: OP full prediction matrix, 4903 users x 2419 items
  - N = 10
  - alpha = 40% candidate filter per user
  - D percentages = 0, 20, 40, 60, 80, 100

The model is the paper's coverage-constrained Top-N 0-1 formulation:

  max sum R[u,i] x[u,i]
  s.t. sum_i x[u,i] = min(N, |I_u|)
       z[i] <= sum_u x[u,i]
       sum_i z[i] >= D
       x,z in {0,1}

By default this script solves the LP relaxation with Gurobi simplex
because the formulation is totally unimodular, so an exact integer
solution is obtained without branch-and-bound. If you want to force a
binary MIP, set USE_LP_RELAXATION = False below.
"""
...
INPUT_XLSX = Path(r"E:\Users\24qxh\Desktop\op\IUIC1_Rui_pred_user_item_FULL_display1.xlsx")
SHEET_NAME = None  # None means use the first sheet.

DATASET_DIR = Path(r"E:\Users\24qxh\Desktop\LETTER-main\LETTER-main\dataset\OP")
...
OUTPUT_XLSX = OUTPUT_DIR / "op_gurobi_N10_D_percent_history_results.xlsx"
OUTPUT_PNG = OUTPUT_DIR / "op_gurobi_N10_D_percent_iteration_plot.png"
...
def main():
    parser = argparse.ArgumentParser(description='Run OP Gurobi exact Top-N coverage solver.')
    ...
    for iteration, pct in enumerate(D_PERCENTAGES):
        raw_target = int(round(num_items * pct / 100.0))
    ...
    meta_row = {
        ...
        "engine": "gurobipy_lp_TU_exact" if USE_LP_RELAXATION else "gurobipy_binary_mip",
