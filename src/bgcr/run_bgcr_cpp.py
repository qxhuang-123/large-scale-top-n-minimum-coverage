...
def run_dataset(dataset: str, n: int, history_interval: int, max_rounds: int):
    ...
    out_xlsx = out_dir / f"{dataset}贪心算法_N{n}_结果.xlsx"
    out_json = out_dir / f"{dataset}贪心算法_N{n}_结果.json"

    print(f"=== {dataset} ===", flush=True)
    print("engine=cpp_pybind11_batch_greedy", flush=True)
    ...
    # 与精确算法一致：构造/类型转换 CSR 后开始计时，并以一个完整运行的逐次交换历史生成各 D 比例结果。
    cpp_user_indptr = np.ascontiguousarray(user_indptr, dtype=np.int32)
    cpp_user_items = np.ascontiguousarray(user_items, dtype=np.int32)
    cpp_user_scores = np.ascontiguousarray(user_scores, dtype=np.float32)
    cpp_item_indptr = np.ascontiguousarray(item_indptr, dtype=np.int32)
    cpp_item_users = np.ascontiguousarray(item_users, dtype=np.int32)
    cpp_item_scores = np.ascontiguousarray(item_scores, dtype=np.float32)
    t0 = time.perf_counter()
    raw = dict(yelp_greedy_core.run_greedy_csr(
        int(num_users), int(num_items),
        cpp_user_indptr, cpp_user_items, cpp_user_scores,
        cpp_item_indptr, cpp_item_users, cpp_item_scores,
        int(n), int(items_with_candidates), 1, int(max_rounds),
    ))
    total_time = time.perf_counter() - t0
    raw_history = list(raw["history"])
    history_rows = [{"数据集": dataset, "迭代次数": int(h["iteration"]), "总体多样性": int(h["diversity"]), "目标函数值": float(h["objective"]), "时间s": float(h["time_sec"])} for h in raw_history]
    percent_rows = []
    for pct in D_PERCENTAGES:
        d_target = int(math.ceil(items_with_candidates * pct / 100.0))
        h = next((h for h in raw_history if int(h["diversity"]) >= d_target), raw_history[-1])
        row = {"数据集": dataset, "D比例": f"D-{pct}%", "D_target": d_target,
               "迭代次数": int(h["iteration"]), "总体多样性": int(h["diversity"]),
               "目标函数值": float(h["objective"]), "准确多样性": float(h["objective"]) / int(raw["total_recs"]) if raw["total_recs"] else 0.0,
               "时间s": float(h["time_sec"]), "target_feasible": bool(int(h["diversity"]) >= d_target)}
        percent_rows.append(row)
        print(row, flush=True)

    pred, diversity, swaps, rounds, obj = float(raw["tpcar_pred"]), int(raw["tpcar_D"]), int(raw["swaps"]), int(raw["rounds"]), float(raw["tpcar_obj"])
    naive_obj, naive_pred, naive_diversity, total_recs = float(raw["naive_obj"]), float(raw["naive_pred"]), int(raw["naive_D"]), int(raw["total_recs"])
    ...
    meta_rows = [
        {
            ...
            "seed": SEED,
            "engine": "cpp_pybind11_batch_greedy_algorithm2",
