...
DEFAULT_N = 10
DEFAULT_SO_N_VALUES = [10, 15, 20, 25, 30]
...
def write_outputs(out_xlsx: Path, out_json: Path, history_rows, percent_rows, summary_rows, meta_rows):
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
        pd.DataFrame(history_rows)[["数据集", "N", "迭代次数", "总体多样性", "目标函数值", "时间s"]].to_excel(
            writer, sheet_name="迭代明细", index=False
        )
        pd.DataFrame(percent_rows)[
            ["数据集", "N", "D比例", "D_target", "迭代次数", "总体多样性", "目标函数值", "准确多样性", "时间s", "target_feasible"]
...
def run_dataset(dataset: str, n: int, history_interval: int, max_rounds: int):
    ...
    return history_rows, percent_rows, summary_rows, meta_rows, out_xlsx, out_json
...
def parse_n_values(text: str) -> list[int]:
    ...
    for part in text.split(","):
        ...
        value = int(part)
        if value <= 0:
            raise ValueError("--n-values 里的 N 必须大于 0")
        values.append(value)
    if not values:
        raise ValueError("--n-values 不能为空")
    return values
...
def parse_args():
    ...
    parser.add_argument("--n", type=int, default=None, help="单个 N；如果同时提供 --n-values，则优先使用 --n-values")
    parser.add_argument("--n-values", default=None, help="多个 N，用逗号分隔，例如 15,20,25,30")
...
def main():
    ...
    print(f"n_values={n_values}", flush=True)
    for n in n_values:
        history_rows, percent_rows, summary_rows, meta_rows, out_xlsx, out_json = run_dataset(
            args.dataset, n, args.history_interval, args.max_rounds
        )
        all_history.extend(history_rows)
        all_percent.extend(percent_rows)
        all_summary.extend(summary_rows)
        all_meta.extend(meta_rows)

    write_outputs(out_xlsx, out_json, all_history, all_percent, all_summary, all_meta)
