...
DATA_PATH = Path(r"E:\Users\24qxh\Desktop\VG\VG_R_ui_UNKNOWN_ONLY_known_ratings_0_display1_1.xlsx")
CACHE_DIR = ROOT / "work" / "vg_exact_cache"
OUT_DIR = ROOT / "outputs" / "vg_exact"
OUT_JSON = OUT_DIR / "vg_exact_N10_D_percent_history_results.json"
OUT_XLSX = OUT_DIR / "vg_exact_N10_D_percent_history_results.xlsx"
OUT_FIG = OUT_DIR / "vg_exact_N10_iteration_curves.png"
...
def _sheet_dimension(path: Path) -> tuple[int, int, str]:
    ...
    if not m:
        raise RuntimeError("Could not read worksheet dimension from xlsx")
...
def load_op_scores(path: Path) -> np.ndarray:
    ...
    cache = CACHE_DIR / "vg_scores_float32.npy"
    tmp_cache = CACHE_DIR / "vg_scores_float32.tmp.npy"
    meta = CACHE_DIR / "vg_scores_meta.json"
    ...
    rows_with_header, cols_with_user_id, sheet_name = _sheet_dimension(path)
    num_users = rows_with_header - 1
    num_items = cols_with_user_id - 1
    print(
        f"fast-reading VG xlsx rows by byte scan: {path}, shape=({num_users}, {num_items}), sheet={sheet_name}",
        flush=True,
    )
    ...
    with zipfile.ZipFile(path) as zf:
        for row_xml in _iter_row_xml(zf, sheet_name):
            ...
            if len(values) <= 1:
                continue
            # The first value is user_id; the remaining values are item scores.
            row_values = np.fromiter((float(x) for x in values[1:]), dtype=np.float32, count=len(values) - 1)
            if row_values.size != num_items:
                raise RuntimeError(f"Row {row_ref} has {row_values.size} item values, expected {num_items}")
...
def build_candidate_cache(scores: np.ndarray):
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / f"vg_cand_frac_0p4_seed{SEED}.npz"
    if cache.exists():
        z = np.load(cache)
        print(f"loaded candidate cache: {cache}", flush=True)
        return z

    print("building VG candidate CSR", flush=True)
...
def write_excel(meta: dict, percent_rows: list[dict], history_rows: list[dict], summary_rows: list[dict]):
    ...
    if ws.max_row >= 2:
        ...
        chart1 = LineChart()
        chart1.title = "VG N=10 总体多样性迭代曲线"
        ...
        chart2 = LineChart()
        chart2.title = "VG N=10 目标函数值迭代曲线"
