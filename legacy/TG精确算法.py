...
CACHE_DIR = ROOT / "work" / "tg_exact_cache"
OUT_DIR = ROOT / "outputs" / "tg_exact"
OUT_JSON = OUT_DIR / "tg_exact_N10_D_percent_history_results.json"
OUT_XLSX = OUT_DIR / "tg_exact_N10_D_percent_history_results.xlsx"
OUT_FIG = OUT_DIR / "tg_exact_N10_iteration_curves.png"
...
def _dimension_to_shape(dim: str) -> tuple[int, int]:
    end = dim.split(":")[-1]
    letters = ""
    digits = ""
    for ch in end:
...
def _iter_row_xml(zf: zipfile.ZipFile, sheet_name: str, chunk_size: int = 16 * 1024 * 1024):
    row_start = b"<row "
    row_end = b"</row>"
    buf = b""
    with zf.open(sheet_name) as fh:
        while True:
            ...
            buf += chunk
            while True:
                start = buf.find(row_start)
                if start < 0:
                    if len(buf) > len(row_start):
                        buf = buf[-len(row_start):]
                    break
                end = buf.find(row_end, start)
                if end < 0:
                    buf = buf[start:]
                    break
                end += len(row_end)
...
def _sheet_shape(path: Path) -> tuple[int, int, str]:
    ...
    if m:
        rows, cols = _dimension_to_shape(m.group(1))
        return rows, cols, sheet_name

    print("discovering TG xlsx shape by scanning row tags", flush=True)
    t0 = time.time()
    header_cells = None
    max_row = 1
    data_rows = 0
    with zipfile.ZipFile(path) as zf:
        for row_xml in _iter_row_xml(zf, sheet_name):
            row_ref = _row_number(row_xml)
            if row_ref == 1:
                header_cells = row_xml.count(b"<c ")
            elif row_ref > 1:
                max_row = max(max_row, row_ref)
                data_rows += 1
                if data_rows % 1000 == 0:
                    print(f"  shape scan rows {data_rows}, elapsed={time.time() - t0:.1f}s", flush=True)
    if header_cells is None or header_cells <= 1:
        raise RuntimeError("Could not infer TG item count from header row")
    return max_row, header_cells, sheet_name

def _row_number(row_xml: bytes) -> int:
    m = re.search(br'<row r="(\d+)"', row_xml[:80])
    return int(m.group(1)) if m else -1


def load_op_scores(path: Path) -> np.ndarray:
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / "tg_scores_float32.npy"
    tmp_cache = CACHE_DIR / "tg_scores_float32.tmp.npy"
    meta = CACHE_DIR / "tg_scores_meta.json"
    ...
    rows_with_header, cols_with_user_id, sheet_name = _sheet_shape(path)
    num_users = rows_with_header - 1
    num_items = cols_with_user_id - 1
    print(
        f"fast-reading TG xlsx rows by byte scan: {path}, shape=({num_users}, {num_items}), sheet={sheet_name}",
        flush=True,
    )

    if tmp_cache.exists():
        tmp_cache.unlink()
    scores = np.lib.format.open_memmap(tmp_cache, mode="w+", dtype=np.float32, shape=(num_users, num_items))
    scores[:] = 0.0

    value_re = re.compile(br"<v>([^<]+)</v>")
    t0 = time.time()
    written_rows = 0
    with zipfile.ZipFile(path) as zf:
        for row_xml in _iter_row_xml(zf, sheet_name):
            ...
            if row_ref <= 1:
                continue
            out_row = row_ref - 2
            values = value_re.findall(row_xml)
            if len(values) == num_items:
                item_values = values
            elif len(values) == num_items + 1:
                item_values = values[1:]
            else:
                raise RuntimeError(f"Row {row_ref} has {len(values)} numeric values, expected {num_items} or {num_items + 1}")
            row_values = np.fromiter((float(x) for x in item_values), dtype=np.float32, count=num_items)
            scores[out_row, :] = row_values
            written_rows += 1
            if written_rows % 500 == 0:
                elapsed = time.time() - t0
                print(f"  xlsx rows {written_rows}/{num_users}, elapsed={elapsed:.1f}s", flush=True)

    scores.flush()
    del scores
    if written_rows != num_users:
        raise RuntimeError(f"Only wrote {written_rows} rows, expected {num_users}")
    if cache.exists():
        cache.unlink()
    tmp_cache.replace(cache)
    meta.write_text(
        json.dumps({"shape": [num_users, num_items], "source": str(path), "source_mtime": newest}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    print(f"cached scores: {cache}, shape=({num_users}, {num_items})", flush=True)
    return np.load(cache, mmap_mode="r")

def build_candidate_cache(scores: np.ndarray):
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / f"tg_cand_frac_0p4_seed{SEED}.npz"
    if cache.exists():
        z = np.load(cache)
        print(f"loaded candidate cache: {cache}", flush=True)
        return z

    print("building TG candidate CSR", flush=True)
...
def write_excel(meta: dict, percent_rows: list[dict], history_rows: list[dict], summary_rows: list[dict]):
    ...
    if ws.max_row >= 2:
        ...
        chart1 = LineChart()
        chart1.title = "TG N=10 总体多样性迭代曲线"
        ...
        chart2 = LineChart()
        chart2.title = "TG N=10 目标函数值迭代曲线"
...
def main():
    ...
    num_users, num_items = scores.shape
    user_items_key = "user_items" if "user_items" in z.files else "user_indices"
    candidate_edges = int(len(z[user_items_key]))
    ...
    percent_rows = make_percent_rows(raw, num_items, total_elapsed)
    summary_rows = [
        {
            "数据集": DATASET,
            "N": N,
