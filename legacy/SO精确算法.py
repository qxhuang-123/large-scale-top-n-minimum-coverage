...
CPP_DIR = Path(
    r"C:\Users\24qxh\Documents\Codex\2026-07-05"
    r"\import-json-import-sys-import-time-4\work\tpcar_cpp39_history"
)
CACHE_DIR = SCRIPT_DIR / "SO贪心算法_cache"
...
try:
    import tpcar_core_compact as tpcar_core  # noqa: E402
except ImportError as exc:
    raise ImportError(
        f"无法导入 SO 精确算法 C++ 模块。请检查 Python 是否为 3.9，以及模块目录是否存在: {CPP_DIR}"
    ) from exc
...
def output_paths(n: int) -> tuple[Path, Path, Path, Path]:
    out_dir = SCRIPT_DIR / "SO精确算法_results"
...
def main():
    ...
    meta = {
        ...
        "seed": SEED,
        "engine": "cpp_pybind11_compact_exact_with_history",
